...
function expandImage(wrapper) {
  ...
  if (classList.contains("img-expanded")) classList.remove("img-expanded");
  else classList.add("img-expanded");
}
...
function writePost() {
  ...
  if (textarea.value.length > 0) {
    ...
    section.innerHTML = `<span class="date">${new Date().toLocaleString()}</span><p>${
      textarea.value
    }</p>`;
